...
var CriteoAdapter = function CriteoAdapter() {
    ...
    function _callBids(params, addBidResponse, done) {
    ...
    function _pushBidRequestEvent(params) {
        ...
        var biddingEventFunc = function () {
            ...
            for (var i = 0; i < bids.length; i++) {
                ...
                var _slot = new Criteo.PubTag.DirectBidding.DirectBiddingSlot(
                    bid.adUnitCode,
                    bid.params.zoneId,
    ...
    function _buildBidObject(bidResponse, slot) {
        ...
        if (bidResponse) {
            ...
            bidObject.ttl = 300;
            bidObject.requestId = slot.requestId;
            bidObject.creativeId = (Date.now + Math.random()).toString();
            bidObject.currency = 'USD';
            bidObject.netRevenue = true;
            ...
            if (slot.nativeCallback && bidResponse['native']) {
                if (typeof slot.nativeCallback !== 'function') {
                    ...
                    window.criteo_pubtag.native_slots['' + bidObject.adId] = {
                        callback: slot.nativeCallback,
                        nativeResponse: bidResponse['native']
                    };
